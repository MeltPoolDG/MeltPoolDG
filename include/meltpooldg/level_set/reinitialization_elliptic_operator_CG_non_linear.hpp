...
namespace MeltPoolDG::LevelSet
{
  ...
  class ReinitializationEllipticOperatorNonLinear : public OperatorMatrixFree<dim, number>
  {
  ...
  public:
     ...
     */
    void
    create_residual(VectorType &rhs, const VectorType &level_set) const;
  ...
  private:
    ...
    VectorizedArrayType
    evaluate_coefficient(const FECellIntegrator<dim, n_components, number> &phi_old,
                         const unsigned int                                 q_index) const;

    /**
     * @brief This evaluates the coefficient derivative for the analytical form.
     *
     * @param psi_old Cell integrator for the old level set values.
     * @param q_index Index of the quadrature point.
     * @return Value of the coefficient derivative.
     */
    template <int n_components>
    VectorizedArrayType
    evaluate_coefficient_derivative(const FECellIntegrator<dim, n_components, number> &phi_old,
                                    const unsigned int q_index) const;

    template <int n_components>
    VectorizedArrayType
    evaluate_coefficient_derivative(const FEFaceIntegrator<dim, n_components, number> &phi_old,
                                    const unsigned int q_index) const;
